...
function renderSinglePunsh(punshes, punshName) {
  for(let i in punshes) {
   ...
   //if we need to cycle through all remove the if
   if(punshName === punshObj['name']){
    console.log(`Name: ${punshObj['name']}`);
    console.log(`Type: ${punshObj['type']}`);
    console.log(`Contents: ${punshObj['contents']}`);
    console.log(`Description: ${punshObj['description']}`);
    }
  }
}
